...
TxT_name = 'D:\MyProjects\FEC\Results\Wavelength_notinterleaved_Hamming_code.txt';
formatspec = [];
for i = 1:3
...
% SNR_good = 15;
BER_noFEC = zeros(nCap,nC+1);
BER_SD = zeros(nCap,nC+1);
BER_HD = zeros(nCap,nC+1);
SNR_mean = 14;
for SNR_good = 14:18
    SNR_bad = -10*log10((2*10^(0.1*-SNR_mean)-10^(0.1*-SNR_good)));
    for iidx = 1:nCap
        
        SNR = [SNR_good SNR_bad 15 15];
        SNR = SNR(1:nC);
...
%                     disp('BER w/o SD Hamming code:');
                    BER_noFEC(iidx,:) = nEr1/size(Cin_mat,2);
%                     disp('BER with SD Hamming code:');
                    BER_SD(iidx,:) = nEr2/size(Cin_mat,2);
                end
...
%                     disp('BER with HD Hamming code:');
                    BER_HD(iidx,:) = nEr2/size(Cin_mat,2);
                end
            case 2
                L_Rx_wdm = zeros(size(x_Tx_wnoise_wdm,1),size(x_Tx_wnoise_wdm,2)*SE);
                for idx = 1:nC
    ...
    fprintf(fileID,formatspec,mean(BER_noFEC));
    fprintf(fileID,formatspec,mean(BER_SD));
    fprintf(fileID,formatspec,mean(BER_HD));
    fprintf(fileID,'\n');
    fclose(fileID);
    disp(['SNR = ',num2str(SNR_good)]);
end